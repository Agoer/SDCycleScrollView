...
UICollectionView *mainView; // 显示图片的collectionView
...
UIImageView *backgroundImageView; // 当imageURLs为空时的背景图
...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame
    ...
    [NSMutableArray arrayWithArray:imageNamesGroup];
...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame
    ...
    [NSMutableArray arrayWithArray:imageURLsGroup];
    return cycleScrollView;
}
...
- (void)setupMainView {
    ...
    [mainView registerClass:[SDCollectionViewCell class]
 forCellWithReuseIdentifier:ID];
    ...
    UIImageView *mengCengImageView = [[UIImageView alloc]
                                      initWithFrame:CGRectMake(0, CGRectGetHeight(self.frame) - 41.8,
                                                               CGRectGetWidth(self.frame), 44)];
    mengCengImageView.image =
    [UIImage imageNamed:@"home_banner_projection_under"];
...
- (void)setCustomPageControlDotImage:(UIImage *)image
                    isCurrentPageDot:(BOOL)isCurrentPageDot {
...
- (void)setImagePathsGroup:(NSArray *)imagePathsGroup {
    ...
    _totalItemsCount = self.infiniteLoop ? self.imagePathsGroup.count * 100
    : self.imagePathsGroup.count;
...
- (void)setupTimer {
    NSTimer *timer =
    [NSTimer scheduledTimerWithTimeInterval:self.autoScrollTimeInterval
                                     target:self
...
- (void)automaticScroll {
    ...
    if (targetIndex >= _totalItemsCount) {
        if (self.infiniteLoop) {
            ...
            [_mainView
             scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex
                                                         inSection:0]
    ...
    [_mainView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex
                                                           inSection:0]
                      atScrollPosition:UICollectionViewScrollPositionNone
                              animated:YES];
}
...
- (int)currentIndex {
    ...
    int index = 0;
    if (_flowLayout.scrollDirection ==
...
- (void)layoutSubviews {
    ...
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        ...
        if (!(self.pageDotImage && self.currentPageDotImage &&
              CGSizeEqualToSize(kCycleScrollViewInitialPageControlDotSize,
                                self.pageControlDotSize))) {
    ...
    _mengCengImageView.frame = CGRectMake(0, CGRectGetHeight(self.frame) - 44,
                                          CGRectGetWidth(self.frame), 44);
...
didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:
